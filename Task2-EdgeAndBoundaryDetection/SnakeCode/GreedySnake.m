...
% Calculate the area of the vicinity   means number of pixels
a = s^2;
...
neighborhood = enrgImg((points(2, modI)-dist):(points(2, modI)+dist), (points(1, modI)-dist):(points(1, modI)+dist));
...
if ceil(a/2) ~= indexMin         % if contour point is not the min move the point.
...
% Use modulo arithmetic since the curve is closed
[modI, modIminus, modIplus] = getModulo(i, n);
if ( c(modI) > c(modIminus) && c(modI) > c(modIplus) && c(modI) > cThreshold && points(6, modI) > imgEnrgT && points(4, modI) ~= 0 )
points(4, modI) = 0;
